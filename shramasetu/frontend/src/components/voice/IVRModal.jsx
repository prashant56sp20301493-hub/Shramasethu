...
import { ivrMenus } from '../../config/ivrMenus';
import { ivrLanguages, ivrTranslations } from '../../config/ivrLanguageConfig';
import { ivrIntentMap, INTENTS } from '../../config/ivrIntentMap';
...
const IVRModal = ({ isOpen, onClose, userRole }) => {
  ...
  const getTranslatedWelcomeMenu = (lang) => {
    ...
    return translationSet.welcomeGreetings[userRole] || translationSet.welcomeGreetings['labour'];
  };
  ...
  }, [currentLanguage, userRole, speak]);
  ...
  const handleLanguageChange = (lang) => {
    ...
    // Speak dynamic changed notice and repeat menu options in the selected language!
  ...
  const handleIntent = useCallback((intent) => {
    ...
    const currentMenu = ivrMenus[userRole] || ivrMenus['labour'];
    const matchedItem = currentMenu.find(item => item.intent === intent);
    ...
    if (matchedItem) {
      ...
      switch (intent) {
        case INTENTS.FIND_JOBS:
          actionText = translations.spokenResponses.findJobs;
          break;
        case INTENTS.MY_WORKS:
          actionText = translations.spokenResponses.myWorks;
          break;
        case INTENTS.ATTENDANCE:
          actionText = translations.spokenResponses.attendance;
          break;
        case INTENTS.WAGES:
          actionText = translations.spokenResponses.wages;
          break;
        case INTENTS.INSURANCE:
          actionText = translations.spokenResponses.insurance;
          break;
        case INTENTS.SUPPORT:
          actionText = translations.spokenResponses.support;
          break;
        case INTENTS.RENTALS:
          actionText = translations.spokenResponses.rentals;
          break;
        case INTENTS.OVERVIEW:
          actionText = translations.spokenResponses.overview;
          break;
        default:
  ...
  const getLocalizedMenuLabel = (labelKey) => {
    ...
    // Explicit mappings to guarantee 100% translation key matches
      ...
      >
        <div className={`w-full max-w-2xl bg-gradient-to-b from-gray-900 via-slate-900 to-black rounded-[2.5rem] border border-white/10 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.8)] flex flex-col my-4 ${isEasyMode ? 'border-amber-400/30' : ''}`}>
          ...
          <div className="px-6 pb-6 flex-1 overflow-y-auto flex flex-col gap-4">
            ...
            {/* Controls Bar (SOS call completely removed as requested) */}
